...
def _tokenize_texts(
    ...
    tok = load_aether_tokenizer(name_or_path=tokenizer_name, cache_dir=tokenizer_cache_dir)
    total = 0
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Tokenize downloaded datasets into train.bin/val.bin using the Aether tokenizer.")
    parser.add_argument("--dataset-root", default="./artifacts/datasets")
    parser.add_argument("--output-dir", default="./artifacts/tokenized")
    parser.add_argument("--tokenizer-name", default=os.environ.get("AETHER_TOKENIZER_NAME", "deepseek-ai/DeepSeek-V3.2"))
