...
@lru_cache(maxsize=1)
def load_deepseek_cuda_kernels(
    verbose: bool = False,
    build_directory: Optional[str] = None,
):
    reason = cuda_extension_unavailable_reason(require_device=False)
    if reason is not None:
        raise RuntimeError(f"DeepSeek CUDA kernels are unavailable: {reason}")
    build_root = build_directory or os.environ.get("DEEPSEEK_KERNEL_BUILD_DIR")
    env = cuda_build_environment()
