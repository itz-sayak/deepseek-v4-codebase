...
from aether_kernels.paged_kv_allocator import PageHandle, PagedKVAllocator
...
from .modeling import CSAAttention, HCAAttention, Aether2BForCausalLM, apply_rope, rope_attention_scale
...
class Aether2BServingEngine:
    def __init__(
        ...
        self.config: Aether2BConfig = model.config
        self.device = torch.device(device or next(model.parameters()).device)
        # Skip bulk .to() for multi-GPU models whose layers are already sharded
        # across devices (_layer_devices set before construction).  Calling .to()
        # on a shared-layer draft model would silently move layers that belong on
        # a secondary GPU back to the primary device, breaking target sharding.
        if getattr(model.model, '_layer_devices', None) is None:
        ...
        # Cache decoded compressed tensors keyed by backing storage identity.
        # Resident compressed caches are read many times between append events;
        # memoizing decode avoids repeated full-cache dequantization per token.
        self._dequant_cache: Dict[Tuple[int, Tuple[int, ...], int, Tuple[int, ...], str], torch.Tensor] = {}
        self._dequant_cache_limit = 128
    ...
    def _read_compressed(
        ...
        if cached is not None:
            return cached
        decoded = self._polar_quant.decode(data, scale, out_dtype=model_dtype)
        if len(self._dequant_cache) >= self._dequant_cache_limit:
            # Simple bounded cache: clear stale entries when capacity is reached.
            self._dequant_cache.clear()
        self._dequant_cache[cache_key] = decoded
        return decoded
    ...
    def _normalize_single_token(self, token_id: Union[int, torch.Tensor]) -> torch.Tensor:
    ...
    def _finalize_logits(self, hidden: torch.Tensor) -> torch.Tensor:
        ...
        mixed = torch.einsum("n,bnd->bd", weights, hidden)
        mixed = self.model.model.final_norm(mixed)
        return self.model.lm_head(mixed)

    def _step_token_internal(
        self,
        token_id: Union[int, torch.Tensor],
        state: Optional[ModelServingState] = None,
        *,
        start_layer: int = 0,
        ...
        token = self._normalize_single_token(token_id)

        if start_layer < 0 or start_layer > len(self.model.model.layers):
            raise ValueError(f"start_layer must be in [0, {len(self.model.model.layers)}], got {start_layer}")

        if start_layer == 0:
            hidden_state = self.model.model.embed_tokens(token).unsqueeze(1).expand(-1, self.config.mhc_expansion, -1).contiguous()
        else:
            if hidden is None:
                raise ValueError("hidden must be provided when start_layer > 0")
            if hidden.ndim != 3:
                raise ValueError("hidden must have shape [B, expansion, hidden_size]")
            hidden_state = hidden

        _layer_devs = getattr(self.model.model, '_layer_devices', None)
        for layer_idx in range(start_layer, len(self.model.model.layers)):
            block = self.model.model.layers[layer_idx]
            layer_state = state.layer_states[layer_idx]
            if _layer_devs is not None:
                dev = _layer_devs[layer_idx]
                if hidden_state.device != dev:
                    hidden_state = hidden_state.to(dev)
                    token = token.to(dev)
                self._move_layer_state_to_device(layer_state, dev)
            hidden_state = self._block_step(block, state, layer_state, layer_idx, token, hidden_state)
            if _layer_devs is not None:
                self._move_layer_state_to_device(layer_state, self.device)

        if _layer_devs is not None and hidden_state.device != self.device:
            hidden_state = hidden_state.to(self.device)

        logits = self._finalize_logits(hidden_state)
        state.token_count += 1
        self._flush_live_page_if_ready(state)
        state.last_logits = logits

        if return_hidden:
            return logits, state, hidden_state
        return logits, state
    ...
    def step_token(self, token_id: Union[int, torch.Tensor], state: Optional[ModelServingState] = None) -> Tuple[torch.Tensor, ModelServingState]:
        return self._step_token_internal(token_id, state)

    @torch.no_grad()
    def step_token_with_hidden(
        self,
        token_id: Union[int, torch.Tensor],
        state: Optional[ModelServingState] = None,
    ) -> Tuple[torch.Tensor, ModelServingState, torch.Tensor]:
        """Step one token and also return the pre-head hidden state.

        The returned hidden has shape [B, mhc_expansion, hidden_size] and can be
        fed into ``step_token_from_hidden`` of a deeper shared-layer target model.
        """
        logits, new_state, hidden = self._step_token_internal(token_id, state, return_hidden=True)
        return logits, new_state, hidden

    @torch.no_grad()
    def step_token_from_hidden(
        self,
        token_id: Union[int, torch.Tensor],
        state: ModelServingState,
        hidden: torch.Tensor,
        start_layer: int,
    ) -> Tuple[torch.Tensor, ModelServingState]:
        """Step one token by executing transformer layers [start_layer, L).

        This is used by shared-layer self-speculative decoding to avoid re-running
        the common draft prefix layers during target verification.
        """
        return self._step_token_internal(token_id, state, start_layer=start_layer, hidden=hidden)
    ...
    def generate_speculative(
        ...
        max_new_tokens: int,
        *,
        draft_engine: Optional["Aether2BServingEngine"] = None,
        draft_steps: int = 5,
        self_spec_layers: Optional[int] = None,
        temperature: float = 1.0,
        top_k: Optional[int] = None,
        eos_token_id: Optional[int] = None,
        adaptive_draft_steps: bool = False,
        draft_quant_bits: Optional[int] = None,
    ) -> List[int]:
        """Integrated speculative generation entrypoint for serving pipelines.

        If ``self_spec_layers`` is provided and ``draft_engine`` is None, this
        builds a shared-layer draft model from the first N target layers and runs
        speculative decoding with the fused shared-layer verification path.

        Parameters
        ----------
        draft_quant_bits : {None, 4, 8}
            Apply TurboQuant KV-cache quantisation to the auto-created draft
            engine.  At long context (≥ 128K tokens) the draft KV-cache
            dominates bandwidth; 4-bit quantisation reduces it ~4×, which can
            flip self-spec from net-slower to net-faster vs the target baseline.
            Has no effect when ``draft_engine`` is passed in directly.
        """
        if len(token_ids) == 0:
            raise ValueError("Generation requires at least one prompt token")

        from .speculative import SpeculativeDecoder, build_self_spec_draft_model

        created_draft_engine: Optional[Aether2BServingEngine] = None
        if draft_engine is None and self_spec_layers is not None:
            draft_model = build_self_spec_draft_model(self.model, int(self_spec_layers))
            # Do NOT call .to() on shared-layer draft — layers are already on the
            # correct devices inherited from the sharded target.  Only move
            # non-shared modules (final_norm, post, lm_head) which are on primary.
            target_layer_devices = getattr(self.model.model, "_layer_devices", None)
            if target_layer_devices is not None:
                draft_model.model._layer_devices = list(target_layer_devices[: int(self_spec_layers)])
            else:
                draft_model = draft_model.to(device=self.device, dtype=self.model.lm_head.weight.dtype)
            created_draft_engine = Aether2BServingEngine(
                draft_model,
                backend=self.backend,
                prefix_manager=None,
                paged_allocator=None,
                device=self.device,
                turbo_quant_bits=draft_quant_bits,
            )
            draft_engine = created_draft_engine
        elif draft_engine is None:
            draft_engine = self
        ...
        decoder = SpeculativeDecoder(
            ...
            top_k=top_k,
            adaptive_draft_steps=adaptive_draft_steps,
        )
        summary = decoder.generate(
            prompt_ids=list(map(int, token_ids)),
            max_new_tokens=max_new_tokens,
            eos_token_id=eos_token_id,
        )

        # Keep a local reference until function return so Python doesn't GC the
        # shared-layer draft engine mid-generation in edge runtimes.
        _ = created_draft_engine
