...
try:
    ...
    raise ModuleNotFoundError("aether_2b requires PyTorch to run") from exc

from .configuration import Aether2BConfig

try:
    from aether_kernels.loader import cuda_extension_unavailable_reason
    from aether_kernels.sparse_attention import tiled_prefill_attention
...
def get_rope_freqs(seq_len: int, config: Aether2BConfig, device: Optional[torch.device] = None) -> torch.Tensor:
    if seq_len <= 0:
...
def rope_attention_scale(config: Aether2BConfig) -> float:
    scaling_type = (config.rope_scaling_type or "none").lower()
...
def _tiled_prefill_enabled(config: Aether2BConfig, device: torch.device) -> bool:
...
def apply_rope(
    ...
    rope_dim: int,
    theta: float,
    config: Optional[Aether2BConfig] = None,
) -> torch.Tensor:
    if rope_dim <= 0:
        return x
...
class ManifoldConstrainedHyperConnection(nn.Module):
    def __init__(self, config: Aether2BConfig, sublayer: nn.Module):
        super().__init__()
        self.config = config
...
class AetherMoE(nn.Module):
    def __init__(self, config: Aether2BConfig, layer_idx: int):
        super().__init__()
        self.config = config
        self.layer_idx = layer_idx
        if config.dense_ffn_intermediate_size > 0:
            # Pure-dense mode: bypass all routing machinery.
            self.dense_ffn = SwiGLU(config.hidden_size, config.dense_ffn_intermediate_size)
            return
    ...
    def forward(
        ...
        if self.config.dense_ffn_intermediate_size > 0:
            out = self.dense_ffn(x)
            if token_mask is not None:
                out = out * token_mask.unsqueeze(-1).to(out.dtype)
            return out, x.new_tensor(0.0)
...
class AetherBlock(nn.Module):
    def __init__(self, config: Aether2BConfig, layer_idx: int):
        ...
        self.moe = ManifoldConstrainedHyperConnection(config, AetherMoE(config, layer_idx))
...
class Aether2BModel(nn.Module):
    ...
    def chunked_forward(
        ...
        Identical to ``forward()`` except each ``AetherBlock`` uses
...
class MTPHead(nn.Module):
    def __init__(self, config: Aether2BConfig, lm_head: nn.Linear):
        super().__init__()
...
class Aether2BForCausalLM(nn.Module):
    def __init__(self, config: Aether2BConfig):
        super().__init__()
        self.config = config
        self.model = Aether2BModel(config)
    ...
    def forward(
        ...
        return AetherOutput(logits=logits, loss=loss, mtp_logits=mtp_logits, balance_loss=balance_loss)
